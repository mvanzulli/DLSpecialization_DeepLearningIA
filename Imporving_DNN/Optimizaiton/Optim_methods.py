...
# In this notebook, you'll gain skills with some more advanced optimization methods that can speed up
#  learning and perhaps even get you to a better final value for the cost function.
# Having a good optimization algorithm can be the difference between waiting days vs. just a few hours
...
# When you take gradient steps with respect to all $m$ examples on each step,
...
# rather than on the whole training set. The code examples below illustrate the difference between stochastic
...
# When the training set is large, SGD can be faster. But the parameters will "oscillate" toward
#  the minimum rather than converge smoothly. Here's what that looks like:


# SGD leads to many oscillations to reach convergence, but each step is a lot faster to compute for SGD than it is for GD,
...
# In practice, you'll often get faster results if you don't use the entire training set, or just one training example,
# to perform each update. Mini-batch gradient descent uses an intermediate number of examples for each step.
...
# - **Shuffle**:
# Create a shuffled version of the training set (X, Y) as shown below.
# Each column of X and Y represents a training example. Note that the random shuffling is done
#  synchronously between X and Y. Such that after the shuffling the $i^{th}$ column of X is the
#  example corresponding to the $i^{th}$ label in Y. The shuffling step ensures that examples
# will be split randomly into different mini-batches.
...
# Implement `random_mini_batches`. The shuffling part has already been coded for you!
# To help with the partitioning step, you've been provided the following code that selects
# the indexes for the $1^{st}$ and $2^{nd}$ mini-batches:
...
def random_mini_batches(X, Y, mini_batch_size=64, seed=0):
    ...
    if m % mini_batch_size != 0:
        mini_batch_X = shuffled_X[:, inc * (k + 1) :]
        mini_batch_Y = shuffled_Y[:, inc * (k + 1) :]
        mini_batch = (mini_batch_X, mini_batch_Y)
        mini_batches.append(mini_batch)
...
# Formally, this will be the exponentially weighted average of the gradient
# on previous steps. You can also think of $v$ as the "velocity" of a ball
# rolling downhill, building up speed (and momentum) according to the direction
# of the gradient/slope of the hill.


### Exercise 3 - initialize_velocity
# Initialize the velocity. The velocity, $v$, is a python dictionary that needs to be initialized with arrays of zeros.
#  Its keys are the same as those in the `grads` dictionary, that is:
# for $l =1,...,L$:
# ```python
# v["dW" + str(l)] = ... #(numpy array of zeros with the same shape as parameters["W" + str(l)])
# v["db" + str(l)] = ... #(numpy array of zeros with the same shape as parameters["b" + str(l)])

# **Note** that the iterator l starts at 1 in the for loop as the first parameters are v["dW1"]
# and v["db1"] (that's a "one" on the superscript).
...
# All parameters should be stored in the `parameters` dictionary.  Note that the iterator `l` starts at
# 1 in the `for` loop as the first parameters are $W^{[1]}$ and $b^{[1]}$ (that's a "one" on the superscript).
...
# - Tuning the optimal $\beta$ for your model might require trying several values to see what works best in terms of
# reducing the value of the cost function $J$.
#
...
# **How does Adam work?**
# 1. It calculates an exponentially weighted average of past gradients, and stores it in variables $v$
# (before bias correction) and $v^{corrected}$ (with bias correction).
# 2. It calculates an exponentially weighted average of the squares of the past gradients, and  stores it in variables $s$
# (before bias correction) and $s^{corrected}$ (with bias correction).
...
# Their keys are the same as for `grads`, that is:
# for $l = 1, ..., L$:
# ```python
# v["dW" + str(l)] = ... #(numpy array of zeros with the same shape as parameters["W" + str(l)])
# v["db" + str(l)] = ... #(numpy array of zeros with the same shape as parameters["b" + str(l)])
...
# Below, you'll use the following "moons" dataset to test the different optimization methods.
...
# On the other hand, Adam clearly outperforms mini-batch gradient descent and Momentum.
...
# During the first part of training, your model can get away with taking large steps,
# but over time, using a fixed value for the learning rate alpha can cause your model
# to get stuck in a wide oscillation that never quite converges. But if you were to
# slowly reduce your learning rate alpha over time, you could then take smaller, slower
# steps that bring you closer to the minimum. This is the idea behind learning rate decay.

# Learning rate decay can be achieved by using either adaptive methods or pre-defined learning rate schedules.

# Now, you'll apply scheduled learning rate decay to a 3-layer neural network in three different
# optimizer modes and see how each one differs, as well as the effect of scheduling at different epochs.

# This model is essentially the same as the one you used before, except in this one you'll be
#  able to include learning rate decay. It includes two new parameters, decay and decay_rate.
...
# For this portion of the assignment, you'll try one of the pre-defined schedules
# for learning rate decay, called exponential learning rate decay. It takes this mathematical form:
...
# Plot decision boundary
plt.title("Model with Gradient Descent optimization and weight decay")
axes = plt.gca()
axes.set_xlim([-1.5, 2.5])
axes.set_ylim([-1, 1.5])
plot_decision_boundary(lambda x: predict_dec(parameters, x.T), train_X, train_Y)
...
# Notice that if you set the decay to occur at every iteration, the learning rate goes
# to zero too quickly - even if you start with a higher learning rate.
# When you're training for a few epoch this doesn't cause a lot of troubles, but when
# the number of epochs is large the optimization algorithm will stop updating. One common
# fix to this issue is to decay the learning rate every few steps. This is called fixed interval scheduling.


### 7.2 - Fixed Interval Scheduling

# You can help prevent the learning rate speeding to zero too quickly by scheduling the exponential learning
# rate decay at a fixed time interval, for example 1000. You can either number the intervals, or divide the
# epoch by the time interval, which is the size of window with the constant learning rate.
...
# Plot decision boundary
plt.title("Model with Gradient Descent optimization and scheduled weight decay")
axes = plt.gca()
axes.set_xlim([-1.5, 2.5])
axes.set_ylim([-1, 1.5])
plot_decision_boundary(lambda x: predict_dec(parameters, x.T), train_X, train_Y)
...
# Plot decision boundary
plt.title(
    "Model with Gradient Descent with momentum optimization and scheduled weight decay"
)
axes = plt.gca()
axes.set_xlim([-1.5, 2.5])
axes.set_ylim([-1, 1.5])
...
# With Mini-batch GD or Mini-batch GD with Momentum, the accuracy is significantly lower than Adam,
# but when learning rate decay is added on top, either can achieve performance at a speed and accuracy
#  score that's similar to Adam.
...
# **Congratulations**! You've made it to the end of the Optimization methods notebook.
