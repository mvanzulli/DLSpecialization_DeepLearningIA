...
# Welcome to your week 4 assignment (part 1 of 2)! 
# Previously you trained a 2-layer Neural Network with a single hidden layer.
...
# It helps grade your work. Please don't change the seed! 
...
#  The documentation for [np.random.randn](https://numpy.org/doc/stable/reference/random/generated/numpy.random.randn.html)
...
# - The model's structure is *[LINEAR -> RELU] $ \times$ (L-1) -> LINEAR -> SIGMOID*. I.e., it has $L-1$ 
...
# - You'll store $n^{[l]}$, the number of units in different layers,
#  in a variable `layer_dims`. For example, the `layer_dims` for last week's Planar Data classification model would have been [2,4,1]: 
# There were two inputs, one hidden layer with 4 hidden units, and an output layer with 1 output unit. This means `W1`'s shape was (4,2), `b1` 
# was (4,1), `W2` was (1,4) and `b2` was (1,1). Now you will generalize this to $L$ layers! 
...
#  You've been provided with the `sigmoid` function which returns **two** items:
...
# You've been provided with the `relu` function. This function returns **two** items: the activation value "`A`" 
# and a "`cache`" that contains "`Z`" (it's what you'll feed in to the corresponding backward function). To use it you could just call:
...
# $A^{[L]} = \sigma(Z^{[L]}) = \sigma(W^{[L]} A^{[L-1]} + b^{[L]})$. (This is sometimes also called `Yhat`, i.e., this is $\hat{Y}$.) 
...
# Remember that backpropagation is used to calculate the gradient of the loss function with respect to the parameters. 

# For those of you who are experts in calculus (which you don't need to be to do this assignment!), 
# the chain rule of calculus can be used to derive the derivative of the loss $\mathcal{L}$ with respect to $z^{[1]}$ 
# in a 2-layer network as follows:

# $$\frac{d \mathcal{L}(a^{[2]},y)}{{dz^{[1]}}} = \frac{d\mathcal{L}(a^{[2]},y)}{{da^{[2]}}}\frac{{da^{[2]}}}{{dz^{[2]}}}\frac{{dz^{[2]}}}{{da^{[1]}}}\frac{{da^{[1]}}}{{dz^{[1]}}} \tag{8} $$

# In order to calculate the gradient $dW^{[1]} = \frac{\partial L}{\partial W^{[1]}}$, use the previous chain rule
#  and you do $dW^{[1]} = dz^{[1]} \times \frac{\partial z^{[1]} }{\partial W^{[1]}}$. During backpropagation, at 
# each step you multiply your current gradient by the gradient corresponding to the specific layer to get the gradient you wanted.

# Equivalently, in order to calculate the gradient $db^{[1]} = \frac{\partial L}{\partial b^{[1]}}$, 
# you use the previous chain rule and you do $db^{[1]} = dz^{[1]} \times \frac{\partial z^{[1]} }{\partial b^{[1]}}$.
...
# you stored a cache which contains (X,W,b, and z). In the back propagation module, 
# you'll use those variables to compute the gradients. Therefore, in the `L_model_backward` function,
#  you'll iterate through all the hidden layers backward, starting from layer $L$. On each step, 
# you will use the cached values for layer $l$ to backpropagate through layer $l$. Figure 5 below shows the backward pass. 
...
#  As seen in Figure 5, you can now feed in `dAL` into the LINEAR->SIGMOID backward function you implemented
#  (which will use the cached values stored by the L_model_forward function). 

# After that, you will have to use a `for` loop to iterate through all the other layers using the 
# LINEAR->RELU backward function. You should store each dA, dW, and db in the grads dictionary. To do so, use this formula : 
